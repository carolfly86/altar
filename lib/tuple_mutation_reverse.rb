class TupleMutationReverse
	...
	def allcolumns_construct(remaining_cols, allColumns_select,allColumns_renamed)
		...
		@allCols_renamed_array = @allColumns_renamed.split(',').map{|v| v.gsub(' ','')}
	end
	...
	def missing_to_excluded(duplicate_removal = false)
		...
		excluded =DBConn.exec(@excluded_query)
		binding.pry if @pk.any?{|pk| pk['col'] == 'e.emp_no' and pk['val'] == '94968'}
		...
		if excluded.count()>0
			...
			excluded_array.uniq{|s| s['mutation_branches']}.each do |br|
				# pp br
				uniq_branches = uniq_branches + br['mutation_branches'].split(',')
			end
			...
			unless uniq_branches.count() == max
				...
				uniq_branches.each do |ubr|
					...
					excluded_nodes = excluded_nodes + @suspicious_nodes.find_all{|nd| nd['branch_name'] == ubr}
					# end
				end
	...
	def satisfied_branch_exonerate(satisfied_branches)
		...
		if satisfied_branches.count()>1
			satisfied_branches.combination(2).each do |br_pair|
				...
				if intersect_cols != [] and col1 != col2
					sn= Hash.new
					# column1 is a subset of column2
					if intersect_cols == col1
						subset_br = br_pair[0]

					end
					# column2 is a subset of column1
					if intersect_cols == col2
						subset_br = br_pair[1]
	...
	def branch_node_pairs()
		...
		@branches.each do |br|
			if @type == 'M'
				...
				@remaining_cols = @remaining_cols - bn_c['columns'] if bn_c['columns'].count ==1
				...
				br.nodes.each_with_index do |nd,idx|
					...
					column_array = nd.columns.uniq{|c| c.hash }
					# binding.pry
					@remaining_cols = @remaining_cols - column_array if column_array.count ==1
			...
			nodes.each do |nd|
				...
				if res.cmd_tuples==0
					...
					insert_query = nodes.map do |nd|
						%Q(INSERT INTO node_query_mapping select #{@test_id} as test_id,
	...
	def eliminate_redundant_tuples(guilty_nodes, guilty_type)
		...
		if guilty_nodes.select{ |gn| gn['branch_name'] =~ /^missing/ or gn['node_name'] =~ /^missing/  } == []
