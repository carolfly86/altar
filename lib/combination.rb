...
module Comb_Calculator
  ...
  def Comb_Calculator.sum_of_comnibations(dominator,s,e)
    ...
    puts "s: #{s}"
    puts "e: #{e}"
    binding.pry if e<0 or s<0 or s>e
    initial = (e-s+1).even? ? 0 : self.num_of_combination(dominator,e)
    # puts initial
    sum = (s+1..e).step(2).reduce(initial){ |sum, n|  sum + self.num_of_combination(dominator+1,n) }
    # { |product, n| product * n }
    # s = s+1
    # while s<=e do
    ...
    if s == 0
      ...
      if s==mid and e == dominator
        ...
        if e-s > dominator-e+s
          ...
          if e < dominator
            sum = sum - self.sum_of_comnibations(dominator,e+1,dominator)
          end
        else
          sum = Comb_Calculator.sum_of_comnibations(dominator,s,e) 
        end
