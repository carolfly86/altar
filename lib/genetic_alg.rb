...
class GeneticAlg
  ...
  def generate_random_program(depth = 0)
    ...
    return generate_rand_equation if (depth == @max_depth - 1) || (aexpr == '')
    depth += 1
    p depth
  ...
  def mutateConstant(parseTree)
    ...
    unless constList.count == 0
      ...
      rand = rand_in_bounds(-10, 10) while rand == constant['val'].to_i
      newVal = rand
      p "newval: #{newVal}"
      mutant['val'] = newVal
  ...
  def mutateColumn(parseTree)
    ...
    unless colList.count == 0
      ...
      rand = @fieldsList[rand(@fieldsList.size)] while rand == col
      newVal = rand
      p "newval: #{newVal}"
  ...
  def search
    population = Array.new(@pop_size) do |_p|
