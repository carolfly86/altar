def create_fix_result_tbl
  ...
  (test_id int, tquery text, fixed_query text,duration int, final_score int)
  )
  DBConn.exec(query)
end

def update_fix_result_tbl(test_id,tquery,fixed_query,duration, score)
...
def create_all_fix_result_tbl
  ...
  (script_name varchar(100), test_id int,
